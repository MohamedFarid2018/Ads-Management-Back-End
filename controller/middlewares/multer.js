...
const VIDEOS_EXT = VIDEOS_UPLOAD_DEFAULT_EXTENSIONS;
        ...
        if (!_.isNil(field) && !_.isNil(field.ext)) {
          ...
          switch (ext) {
            case ALLOWED_FILE_TYPES.IMAGES:
              ...
              break;
            case ALLOWED_FILE_TYPES.VIDEOS:
              extensions.push(...VIDEOS_EXT);
              break;
        ...
        if (e) {
          switch (e.code) {
            case "LIMIT_UNEXPECTED_FILE":
              ...
              error.arMessage = `يجب أن يكون عدد الملفات أقل من أو يساوي ${count}`;
              break;
            case "LIMIT_FILE_SIZE":
              error.message = `File size must be less than or equal to ${size} bytes`;
              error.arMessage = `يجب أن يكون حجم الملف أقل من أو يساوي ${size} بايت`;
              break;
            case 23:
              error.message = e.message;
              error.arMessage = e.message;
              break;
            default:
              error.message = "Error while uploading files";
              error.arMessage = "خطأ أثناء تحميل الملفات";
          }
