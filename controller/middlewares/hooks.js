...
module.exports = (req, res, next) => {
    ...
    req.validate = (schema, data, strict = true) => {
        ...
        if (!valid) {
            ...
            throw new ValidationError(null, ajv.errors, ajv.errors);
        }
    };
    ...
    const getAllowedFields = (userFields, permissionAttrs) => {
        ...
        if (fieldsType === 0) {
            throw new ValidationError(0, 'Fields cannot have a mix of inclusion and exclusion', 'لا يمكن أن تحتوي الحقول على مزيج من التضمين والاستبعاد');
        }
    ...
    req.authorize = async (user, resource, access, predicate) => {
        try {
            ...
            if (_.intersection(nAccess, accessType).length !== nAccess.length) {
                throw new ValidationError(0, 'authorize hook: invalid access type', 'نوع وصول غير صالح');
            }
